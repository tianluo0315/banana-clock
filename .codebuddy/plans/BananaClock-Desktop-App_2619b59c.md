---
name: BananaClock-Desktop-App
overview: 创建一个基于番茄工作法的Windows桌面应用，使用Electron+React+TypeScript+SQLite技术栈，包含专注计时器、任务管理、数据统计三大核心模块。
design:
  architecture:
    framework: react
    component: shadcn
...
todos:
  - id: project-init
    content: 初始化Electron + React + TypeScript项目结构，配置构建工具和依赖
    status: completed
  - id: database-setup
    content: 设计并实现SQLite数据库结构，创建数据模型和迁移脚本
    status: completed
    dependencies:
      - project-init
  - id: electron-core
    content: 实现Electron主进程功能：窗口管理、系统托盘、IPC通信
    status: completed
    dependencies:
      - project-init
  - id: ui-foundation
    content: 搭建React应用基础结构，创建共享组件库和设计系统
    status: completed
    dependencies:
      - project-init
  - id: timer-module
    content: 实现专注计时器模块：倒计时逻辑、控制按钮、音效播放
    status: completed
    dependencies:
      - database-setup
      - ui-foundation
  - id: task-module
    content: 实现任务管理模块：任务CRUD操作、状态管理、卡片组件
    status: completed
    dependencies:
      - database-setup
      - ui-foundation
  - id: stats-module
    content: 实现数据统计模块：图表展示、历史记录、数据聚合
    status: completed
    dependencies:
      - database-setup
      - ui-foundation
  ...
  - id: integration-testing
    ...
    dependencies:
      ...
      - stats-module
  - id: packaging-deploy
    content: 使用electron-builder打包Windows安装包，创建发布版本
    status: completed
    dependencies:
      - integration-testing
  - id: de924128
    content: 支持后续版本迭代，迭代内容包括bug修复、需求优化、性能优化等。
    status: completed
---
